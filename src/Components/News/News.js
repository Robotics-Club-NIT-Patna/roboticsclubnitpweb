          ...
          {updatedBlogsData.map((obj, i) => {
            return (
              <li>
                ...
                <article>{i===0 ? <img  src={newblog} style={{width : "25%" , position : "absolute", zIndex : "30", left : "-10px", top : "-230px"}} /> : ""}
                  <a href={obj.bloglink}>
                    <h3>{obj.blogTitle}</h3>
                    <p>{obj.tempContent}</p>
                  </a>
                  <div className="share_date">{obj.blogDate}
                    <a title="Share this blog" onClick={() => handleSharex(i)} style={{ cursor: "pointer" }}>
                      <span className="share_dev">Share <i className="fa fa-share"></i></span>
                    </a>
                    ...
                    />
                    <div
                      style={blogShareId == i ? { transform: "scale(1)", position: "absolute", right: "0px", bottom: "-7px", fontSize: "20px", color: "red", zIndex: "31", cursor: "pointer", transition: "0.8s", background : "#ffb5b5", padding : "0px 5px", width : "40px", borderRadius : "0px 0px 50% 50%" } : { transform: "scale(0)" }}
